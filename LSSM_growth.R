...
#   - Hourly growth is necessary to account for biochemical changes
# 
# ## Updates: 
...
DLI_fact  <- DLI_scale( hourly_PAR )
length(temp_fact)

# Show the inhibition factors 
plot( temp_fact, type = 'l' )

# NB: This scaling function needs correcting so it's on the scale [0,1] before
# inhibited growth model can be run
...
r <- 0.065 #vary this to explore growth curve.
x <- NULL
...
# #   Mooring 5 is 'reference' and mooring 7 is 'inside' the archipelago
