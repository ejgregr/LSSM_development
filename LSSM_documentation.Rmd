...
Each box tracked time-dependent physical state variables relevant to estuarine stratification and mixing: temperature (T, in C), salinity (S, in PSU), and water density (rho, in kg/m³), with density calculated at each time step using the UNESCO equation of state (rho = f(T, S)). These variables formed the basis for resolving both horizontal and vertical exchange dynamics. Optionally, additional state variables such as dissolved oxygen, dissolved inorganic carbon (DIC), or nutrients (e.g., nitrate, phosphate) could be included, depending on the modeling objectives. The model also allowed for the calculation of light attenuation, vertical density gradients, and buoyancy frequencies to inform stability and mixing thresholds.


# Marine characterisation

## Temperature and salinity
BATI has 2023 data from 8 moorings in the region. Main ones of interest to this project are B5, which is in Blackfish Sound, and B7, which is in a sheltered location west of Gilford Island, not unlike the Village Island basin. 
...
MAPP also has 5 moorings in the area. Of interest may be M5 (in QCS) and M4 (at the entrance to Knight Inlet). The MAPP moorings include a StarOddi only, collecting T/S at surface (~ 1 m). These data have not yet been processed.
...
Alkalinity is needed to calculate pH. Fortunately, it has a strong relationship with salinity. BATI moorings have salinity, so can get TA from salinity relationship. THis has been derived by Evans for various locations in the eastern Pacific. For example:
  ...
  TA = 48.771 * S + 606.23 umol $kg-1$ (Evans et al. 2015, for SE Ak)  
  TA = 59.88  * S + 278.79 umol $kg-1$ (Evans et al. 2019, for the Northern Salish Sea)  

The fit is tight (r2 = 0.94, root mean square error = 17.21 umol kg$-1$), and was calculated using only salinity data < 33.6.
...
Between October 2017 and October 2019, underway surface water measurements were collected from the Alaska Marine Highway System M/V Columbia during 135 service route transits. The vessel sailed south from Ketchikan every Wednesday, arrived in Bellingham on Friday, returned to Ketchikan on Sunday. Measurements of temperature, salinity, dissolved oxygen content, and CO2 partial pressure were made every 2 mins on seawater drawn into the vessel from a ~2 m intake.

A subset of these data were obtained from Hakai, restricted spatially to the southeastern portion of Queen Charlotte Strait, adjacent to our area of interest (i.e., between 50.6 and 50.7N, and -126.7 and -126.8W). This yielded 676 records with the attributes date/time, lat, lon, T, S, and several different pCO2 measures. 
...
I = mol^3/m^2/d, photon concentration (= irradiance)
...
# Ocean chemistry
A critical aspect of estimating the change in pH attributable to kelp. With the data described above, we used carb() from the seacarb package to estimate the change in pH attributable to kelp.
Some general estimates in NEP coastal waters for sanity checking (from ChatGPT):
Typical Alkalinity : 1800 - 2300 umol/kg  More in upwelled waters. Varies due to freshwater input, biological processes, and upwelling.  
Typical DIC        : 2000 - 2500 umol/kg. Upwelled waters at high end.  
Typical pCO2       : 300 - 450 utm.       >1000 in upwelled waters. This is near equilibrium w atm.  
Typical pH         : 7.6 to 8.2           Depends on upwelling, biol activity (e.g., ecosystem metabolism - Lowe et al. 2019)  
...
## Field data 
Field observations of nereo helped parameterise growth and distribution
...
<!--
# grow_dat was created to feed the DEB. This is postponed for now as have no well-calibrated DEB. 
